...
export default function BlogPage() {
  ...
  return (
    <main className="min-h-screen bg-[#FFFFFF] pt-16">
      ...
      {/* Header Section */}
      <section className="border-b-2 border-[#1a1a1f] p-8 lg:p-16 flex flex-col lg:flex-row justify-between items-end gap-8">
        <div>
        ...
        {posts.length > 0 ? (
          posts.map((post, index) => (
            <Link 
              key={post.slug} 
              href={`/blog/${post.slug}`}
              className="group border-b-2 border-[#1a1a1f] md:border-r-2 md:even:border-r-0 lg:border-r-2 lg:[&:nth-child(3n)]:border-r-0 p-8 lg:p-12 hover:bg-[#00FF00] transition-colors duration-300 flex flex-col justify-between min-h-[400px]"
            >
              <div>
                <div className="flex justify-between items-start mb-8">
                  ...
                  <div className="w-2 h-2 rounded-full bg-[#1a1a1f] group-hover:bg-white transition-colors" />
                </div>
              ...
              <div className="mt-12 flex items-center gap-4">
                ...
                <div className="h-px flex-1 bg-[#1a1a1f]/10 group-hover:bg-white/30 transition-colors" />
              </div>
            </Link>
          ))
        ) : (
          <div className="col-span-full p-24 text-center border-b-2 border-[#1a1a1f]">
